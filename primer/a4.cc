...
vector<int> RadixSort(vector<int> input, int n) {
    ...
    for (int d = 1; d <= MaxBit(input); d++) {
        ...
        /*
         * 比如某次经过上面统计后结果为：[0, 2, 3, 3, 0, 0, 0, 0, 0,
         * 0]则经过下面计算后 结果为： [0, 2, 5, 8, 8, 8, 8, 8, 8,
         * 8]但实质上只有如下[0, 2, 5, 8, 0, 0, 0, 0, 0, 0]中
         ...
         * 元素的位置时，位记数器是从大到到小（count[digit(arr[i],
         * d)]--）的方式来处
...
void a4() {
    vector<int> arr = {17, 29, 11, -9, 32, 53, 23, 66, 71, 43, -3,
                       96, 76, 37, 42, 97, -1, 25, 56, 94, 10};
    insertSort(arr);
}